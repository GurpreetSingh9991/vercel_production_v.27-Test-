...
const Psychology: React.FC<PsychologyProps> = ({ trades }) => {
  const stats = useMemo(() => {
    ...
    // Filter trades that have plan compliance metadata
    ...
    const disciplineScore = tradesWithPlan.length > 0 
      ? Math.round((followedPlan.length / tradesWithPlan.length) * 100) 
      : 0;

    const followedPnL = followedPlan.reduce((sum, t) => sum + (Number(t.pnl) || 0), 0);
    const followedAvg = followedPlan.length > 0 ? followedPnL / followedPlan.length : 0;
    const followedWR = followedPlan.length > 0 
      ? (followedPlan.filter(t => t.pnl > 0).length / followedPlan.length) * 100 
      : 0;

    const notFollowedPnL = notFollowed.reduce((sum, t) => sum + (Number(t.pnl) || 0), 0);
    const notFollowedAvg = notFollowed.length > 0 ? notFollowedPnL / notFollowed.length : 0;
    const notFollowedWR = notFollowed.length > 0 
      ? (notFollowed.filter(t => t.pnl > 0).length / notFollowed.length) * 100 
      : 0;

    // Refined Logic: Only calculate deltas if we have samples for both categories
    const hasBaseline = followedPlan.length > 0 && notFollowed.length > 0;
    
    // "Execution Edge" is the average performance boost from being disciplined
    const impactPerTrade = hasBaseline ? Math.max(0, followedAvg - notFollowedAvg) : 0;
    
    // "Opportunity Leak" is the total potential profit missed (Average Disciplined PnL - Actual Deviated PnL) * count
    const totalImpactValue = hasBaseline 
      ? Math.max(0, (followedAvg * notFollowed.length) - notFollowedPnL)
      : 0;

    // Emotional Performance Logic
    const byEmotion: Record<string, { trades: Trade[], wins: number, totalPnL: number }> = {};
    trades.forEach(t => {
      const states = t.psychology?.states || [];
      states.forEach(emotion => {
    ...
    const emotionStats = Object.entries(byEmotion).map(([emotion, data]) => ({
      ...
      verdict: (data.totalPnL / data.trades.length) > 0 ? '✅ TRADE' : (data.totalPnL / data.trades.length < -50 ? '🚫 NEVER TRADE' : '🚫 DON\'T TRADE')
    })).sort((a, b) => b.avgPnL - a.avgPnL);

    const bestEmotion = emotionStats[0];
    const worstEmotion = emotionStats[emotionStats.length - 1];
    const emotionalDelta = (bestEmotion?.avgPnL || 0) - (worstEmotion?.avgPnL || 0);
    ...
    trades.forEach(t => {
      const day = t.date;
      dateMap[day] = dateMap[day] || 0;
      if (t.mistakes && t.mistakes.length > 0) {
        t.mistakes.forEach(m => {
          allMistakes.push(m);
          totalMistakeCost += (m.estimatedCost || 0);
          dateMap[day] += 1;
        });
      }
    });
    ...
    const impactMap: Record<string, number> = { LOW: 0, MEDIUM: 0, HIGH: 0, CRITICAL: 0 };
    ...
    allMistakes.forEach(m => {
      ...
      categoryMap[cat].cost += (m.estimatedCost || 0);
      categoryMap[cat].impactScores.push(impactValue[imp]);
      impactMap[imp] += 1;
    });
    ...
    const impactDistribution = Object.entries(impactMap).map(([name, value]) => ({ name, value }));
  ...
  const scoreColor = stats.disciplineScore >= 80 ? 'text-emerald-500' : stats.disciplineScore >= 60 ? 'text-amber-500' : 'text-rose-500';
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-700">
      ...
      {/* Discipline Hero Section */}
      <section className="space-y-6">
        <div className="apple-glass p-6 sm:p-12 rounded-[2.5rem] sm:rounded-[3.5rem] relative overflow-hidden group border-white/60 ios-shadow">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 sm:gap-10">
            <div className="relative w-32 h-32 sm:w-40 sm:h-40 flex items-center justify-center shrink-0">
               <svg className="w-full h-full -rotate-90" viewBox="0 0 160 160">
                 ...
                 <circle cx="80" cy="80" r={radius} fill="transparent" stroke="currentColor" strokeWidth="12" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" className={`transition-all duration-1000 ${scoreColor}`} />
               </svg>
            ...
            <div className="flex-1 space-y-4 text-center md:text-left">
              <h2 className="text-[8px] sm:text-[10px] font-black text-black/20 uppercase tracking-[0.4em] mb-2">Behavioral Performance Score</h2>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-4 pt-4">
                 <div className="ceramic-white px-4 sm:px-5 py-2.5 sm:py-3 rounded-2xl border border-black/5 shadow-sm">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="apple-glass p-6 rounded-[2rem] border-emerald-500/20 bg-emerald-500/[0.02]">
               ...
               <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Net Profit</span>
                    <span className="text-sm font-black text-emerald-600">{stats.followedPnL >= 0 ? '+' : ''}${Math.round(stats.followedPnL).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Win Rate</span>
                    <span className="text-sm font-black text-black">{stats.followedWR.toFixed(0)}%</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Avg. Yield</span>
                    <span className="text-sm font-black text-black">${Math.round(stats.followedAvg).toLocaleString()}</span>
                  </div>
               </div>
            </div>
            ...
            <div className="apple-glass p-6 rounded-[2rem] border-rose-500/20 bg-rose-500/[0.02]">
               ...
               <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Net Profit</span>
                    <span className="text-sm font-black text-rose-600">{stats.notFollowedPnL >= 0 ? '+' : ''}${Math.round(stats.notFollowedPnL).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Win Rate</span>
                    <span className="text-sm font-black text-black">{stats.notFollowedWR.toFixed(0)}%</span>
                  </div>
                  <div className="flex justify-between items-end">
                    <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Avg. Yield</span>
                    <span className="text-sm font-black text-black">${Math.round(stats.notFollowedAvg).toLocaleString()}</span>
                  </div>
               </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="apple-glass p-8 rounded-[2rem] h-full flex flex-col justify-center gap-6 border-white/60">
             <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-black text-black tracking-tight">Following your plan adds <span className="text-emerald-600">${Math.round(stats.impactPerTrade).toLocaleString()}</span> to your average trade yield.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-black text-black tracking-tight">You left <span className="text-rose-600">${Math.round(stats.totalImpactValue).toLocaleString()}</span> on the table by deviating from your rules.</p>
                </div>
             </div>
          </div>
        </div>
      ...
      {/* Friction Analysis Section (Mistakes) */}
      <section className="space-y-8 pt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <section className="lg:col-span-12">
            ...
            <div className="apple-glass p-5 sm:p-8 rounded-[2.5rem] ios-shadow border-white h-[300px]">
          ...
          <section className="lg:col-span-8">
            ...
            <div className="apple-glass p-5 sm:p-8 rounded-[2.5rem] ios-shadow border-white h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <section className="lg:col-span-4">
            ...
            <div className="apple-glass p-5 sm:p-8 rounded-[2.5rem] ios-shadow border-white h-[400px] flex flex-col items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie 
                  ...
                  >
                    {stats.impactDistribution.map((entry: any, index: number) => (
                      <Cell key={`cell-${index}`} fill={COLORS.MISTAKES[entry.name as keyof typeof COLORS.MISTAKES] || '#000'} />
                    ))}
                  </Pie>
                  <Tooltip cursor={false} content={<CustomTooltip />} />
                </PieChart>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 mt-4">
                {['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].map(lvl => (
                  <div key={lvl} className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS.MISTAKES[lvl as keyof typeof COLORS.MISTAKES] }} />
                    <span className="text-[8px] font-black uppercase text-slate-400 tracking-widest">{lvl}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="space-y-8 pt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-black/5">
                  ...
                  {stats.emotionStats.length === 0 && (
                    <tr>
                      <td colSpan={5} className="px-6 py-12 text-center text-[10px] font-bold text-black/20 uppercase tracking-widest">
                        Insufficient psychological data points recorded.
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          {/* Emotional Yield Chart */}
          <section className="lg:col-span-5">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-6 ml-2 flex items-center gap-2.5">
               <div className="w-1 h-3.5 bg-black rounded-full" /> Average Yield by State
            </h3>
            <div className="apple-glass p-5 sm:p-8 rounded-[2rem] ios-shadow border-white h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.emotionStats} layout="vertical" margin={{ top: 10, right: 30, left: 40, bottom: 0 }}>
                  ...
                  <Bar dataKey="avgPnL" radius={[0, 6, 6, 0]} barSize={24}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-4">
          ...
          <div className="apple-glass p-8 rounded-[2rem] border-emerald-500/20 bg-emerald-500/[0.02] space-y-4">
             ...
             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-4">Trade when feeling:</p>
          ...
          <div className="apple-glass p-8 rounded-[2rem] border-rose-500/20 bg-rose-500/[0.02] space-y-4">
             ...
             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-4">Never trade when:</p>
             <ul className="space-y-3">
